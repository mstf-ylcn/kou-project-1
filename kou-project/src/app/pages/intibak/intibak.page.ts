...
@Component({
  selector: 'app-intibak',
  templateUrl: './intibak.page.html',
  styleUrls: ['./intibak.page.scss'],
})
export class IntibakPage implements AfterViewInit {
 ...
 dersler=[];
   belgeler = [
     ...
     {belge_Ad:'Ders İçeriği',url:'',dosya_Ad:"",index:-1,isLoad:0},
...
addItem()
{
  if(this.ders!="")
  {
    var index=this.tercihler.indexOf(this.ders);

    if(index==-1)
    {
    this.tercihler.push(this.ders);
    }
  }
...
uploadImage(data,dataType,type,index,name)
{
 ...
 var upload=this.storage.upload('intibak/'+`${type}/`+`${this.userService.userData.name}_`+`${this.time}`+type,this.data);
  upload.then(res=>{
...
async createPdf()
{
  ...
  this.pdfName=this.userService.userData.tc+'_'+this.userService.userData.name+'_'+date+'intibak.pdf';
...
async  readPdf  ()  {
  ...
  var c =this.b64toBlob(data.data,'application/pdf');
    var upload= this.storage.upload(`intibak/.pdf/${this.pdfName}`,c);
    upload.then(res=>{
      res.task.snapshot.ref.getDownloadURL().then(downloadableUrl=>{
