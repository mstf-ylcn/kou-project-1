...
@Component({
  selector: 'app-yaz-okulu',
  templateUrl: './yaz-okulu.page.html',
  styleUrls: ['./yaz-okulu.page.scss'],
})
export class YazOkuluPage implements OnInit {
  ...
  content_class6="none";
   ...
   belgeler = [
     ...
     {belge_Ad:'Transkript',url:'',dosya_Ad:"",index:-1,isLoad:0},
    ];
...
selectFakulte(event)
{
 ...
 this.disableDers="true";
 this.ders=null
}
selectBolum(event)
{
  this.bolum=event.detail.value;
  this.disableDers="false";
  console.log(this.fakulteBolum);
  console.log(this.fakulteBolum[this.bolum]);

  this.fireDb.database.ref().child("dersler/"+`${this.bolum}`).on('value',data=>{
  this.dersler=data.val();
...
addItem()
{
  if(this.tercihler.length==0 && this.bolum!="" && this.ders!="" )
  {
  this.tercihler.push({fakulte:this.fakulte,bolum:this.bolum,ders:this.ders});
  }
  else if(this.bolum!="" && this.ders!="")
  {
    var index=this.tercihler.findIndex(a=>a.bolum===this.bolum);
    var index2=this.tercihler.findIndex(a=>a.ders===this.ders);
...
async deleteItem(index,ders,bolum,fakulte) {
  ...
  const alert = await this.alertController.create({
    ...
    `<img  src="../../../assets/pencil2.png"  alt="">`+
    `<ion-label class="text">${ders}</ion-label>`+'<br>',
    buttons: [
      {
        text: 'İptal',
        role: 'cancel',
...
uploadImage(data,dataType,type,index,name)
{
 ...
 var upload=this.storage.upload('yazokulu/'+`${type}/`+`${this.userService.userData.name}_`+`${this.time}`+type,this.data);
  upload.then(res=>{
...
async createPdf()
{
  ...
  this.pdfName=this.userService.userData.tc+'_'+this.userService.userData.name+'_'+date+'yaz_okulu.pdf';
...
async  readPdf  ()  {
  ...
  var c =this.b64toBlob(data.data,'application/pdf');
    var upload= this.storage.upload(`yazokulu/.pdf/${this.pdfName}`,c);
    upload.then(res=>{
      res.task.snapshot.ref.getDownloadURL().then(downloadableUrl=>{
